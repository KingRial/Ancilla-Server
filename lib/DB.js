...
class DB {
	constructor( oOptions ){
		...
		fs
			...
			.forEach( function( sFile ){
				_DB._oSequelize.import( path.join( _sRelativeModelPath, sFile ) );
			});
		// Umzug
	...
	*/
	__startBreezeService(){
		...
		// Init Breeze routes
		_oApp.get( path.join( this.__oOptions.sBreezeRequestPath, 'Metadata' ).replace(/[\\$'"]/g, "\\$&"), function ( req, res, next ){
console.error( 'TODO: get metdata' );
				/*
        try {
    ...
    _oApp.get( path.join( this.__oOptions.sBreezeRequestPath, ':entity' ).replace(/[\\$'"]/g, "\\$&"), function( req, res, next ){
console.error( 'TODO: get entity' );
/*
...
*/
    ...
    _oApp.post( path.join( this.__oOptions.sBreezeRequestPath, 'Save' ).replace(/[\\$'"]/g, "\\$&"), function ( req, res, next ) {
...
*/
    ...
    _oApp.post( path.join( this.__oOptions.sBreezeRequestPath, 'Purge' ).replace(/[\\$'"]/g, "\\$&"), function( req, res, next ){
...
*/
    ...
    _oApp.post( path.join( this.__oOptions.sBreezeRequestPath, 'Reset' ).replace(/[\\$'"]/g, "\\$&"), function( req, res, next ){
