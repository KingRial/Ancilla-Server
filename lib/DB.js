...
class DB {
	...
	__initEnvSequelize(){
		...
		fs
			...
			.forEach( function( sFile ){
				_DB._oSequelize.import( path.join( _DB.__oOptions.sModelsDir, sFile ) );
			});
	...
	*/
	...
	transaction( fFunction ){
		return this._oSequelize.transaction( function( oTransaction ){
			return fFunction( oTransaction );
		});
	}
}
