...
*/
var GatewayEndpoint = require('./Endpoint.js');
...
class Gateway extends EventEmitter {
  constructor( aEndpointsOptions, oGatewayOptions  ){
    // Calling super constructor
    super();
  	// Init Options
  	aEndpointsOptions = aEndpointsOptions || [];
    oGatewayOptions = _.extend( {
      oLogger: null
    }, oGatewayOptions );
    // Init logger and extends loggind methods on this class
		var _oLogger = ( oGatewayOptions.oLogger ? oGatewayOptions.oLogger : new Logger() );
		_oLogger.extend( this );
    oGatewayOptions.oLogger = _oLogger;
  	// Init properties
  	this._aGWEndpoints = {};
    this.__oOptions = oGatewayOptions;
  	// Creating Endpoints
    for( let [ _iIndex, _oGatewayOption ] of Object.entries( aEndpointsOptions ) ){
  		var _oCurrentGWEndpointOptions = _.extend( {
  ...
  write( sGWEndpointID, oData, sEncoding, fCallback ){
  	if( this._aGWEndpoints[ sGWEndpointID ] ){
  		...
  		this.error( '[ Gateway ] Unable to write on endpoint: "%s": missing endpoint...', sGWEndpointID );
  	}
  }
   ...
   *   Gateway.write( 0, oData );
   */
  addEndpoint( oEndpointOptions ){
    oEndpointOptions = _.extend( {
      oLogger: this.__oOptions.oLogger
    }, oEndpointOptions );
    var _oGateway = this;
  	var _oGWEndpoint = new GatewayEndpoint( oEndpointOptions );
  	if( !_oGWEndpoint ){
  		this.error( '[ Gateway ] Unable to create Gateway endpoint with the following options: %s', oEndpointOptions );
  	} else {
  		// Handling ready event
  		_oGWEndpoint.on( 'ready', function() {
  			var _aEndpoints = _oGateway.getEndpoints();
  			var _bReady = true;
        // for( let _oEndpoint of _aEndpoints ){ // Why this "for" doesn't work ?
        for( let [ _iIndex, _oEndpoint ] of Object.entries( _aEndpoints ) ){
  ...
  */
  		_oGWEndpoint.on('data', function( oData, iSocketIndex ) {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] Data received: "%s"...', this.getID(), oData.toString('hex') );
  ...
  */
  		_oGWEndpoint.on('drain', function( oData ) {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] Data drained: "%s"...', this.getID(), oData.toString('hex') );
  ...
  */
  		_oGWEndpoint.on('timeout', function() {
  			//_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] Timedout...', oGWEndpoint.getID() );
  ...
  */
  		_oGWEndpoint.on('close', function( iSocketIndex, oEndpoint ) {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] Socket "%s" closed...', this.getID(), iSocketIndex );
  ...
  */
  		_oGWEndpoint.on('error', function( oError ) {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] %s...', this.getID(), oError );
  ...
  */
  		_oGWEndpoint.on('connection', function( oSocket ) {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] connection received from %s:%s...', this.getID(), oSocket.remoteAddress, oSocket.remotePort );
  ...
  */
  		_oGWEndpoint.on('connect', function() {
  			_oGateway.debug('[ Gateway ][ Endpoint ( ID: %s ) ] connected estabilished with %s:%s...', this.getID(), this.getHost(), this.getPort() );
  	...
  	if( !this._aGWEndpoints[ _oGWEndpoint.getID() ] ){
  		...
  		this.error( '[ Gateway ] Unable to add Gateway endpoint; ID: "%s" already used', _oGWEndpoint.getID() );
  	}
  	return _oGWEndpoint;
  }
