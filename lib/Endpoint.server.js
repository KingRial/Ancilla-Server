...
/**
 * Method used to write a buffer on the connected scokets
 *
 * @method    write
 * @public
 *
 ...
 *
 * @example
 *   Endpoint.write( oBuffer, sSocketID, sEncoding );
 */
	write( oBuffer, sSocketID, sEncoding ){
    let _aWritePromises = [];
    ...
    for( let _sConnectedSocketID in _oSocketsToWriteIn ){
      if( _oSocketsToWriteIn.hasOwnProperty( _sConnectedSocketID ) ){
        let _oSocketToWriteIn = _oSocketsToWriteIn[ _sConnectedSocketID ];
         ...
         _aWritePromises.push( this.writeSocket( _oSocketToWriteIn, oBuffer, sEncoding ) );
      }
    }
 ...
 *
 * @example
 *   Endpoint.writeSocket( oSocket, oBuffer, sEncoding );
 */
  writeSocket( oSocket, oBuffer, sEncoding ){
    let _Server = this;
