...
class Endpoint extends EventEmitter {
	...
	getStatus(){
    return this.__iStatus;
  }

	setStatus( iStatus ){
    this.__iStatus = iStatus;
  }

  setStatusMask( mask ){
    this.__iStatus = Tools.setStatusByteByMask( this.__iStatus, mask );
  }

	unsetStatusMask( mask ){
    this.__iStatus = Tools.unsetStatusByteByMask( this.__iStatus, mask );
  }

  checkStatusMask( mask ){
    return Tools.checkStatusByteByMask( this.__iStatus, mask );
  }

	getEvents(){
		return this.getConfig().aOfferedEvents;
	}
...
// TODO: should accept a constructor, a singleton or an object filled by options
	...
	ready(){
		return ( this.__oPromiseReady ? this.__oPromiseReady : Bluebird.resolve() );
	}
...
// TODO: check this promise chain
	...
	__bUseDatagrams(){
		return ( this.__oOptions.aDatagrams ? true : false );
	}
