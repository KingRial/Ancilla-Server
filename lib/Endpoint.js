...
class Endpoint extends EventEmitter {
	...
	__createEmitter(){
		...
		if( !this.getEmitter() ){
			switch( this.getConnectionType() ){
				...
				break;
				default:
					_oGatewayEndpoint.error( '[ Endpoint ( ID: %s ) ] Error: unable to determine endpoint type from "%s"', _oGatewayEndpoint.getID(), this.getConnectionType() );
				break;
			}
			if( Emitter ){ //if Class exists
				this.__iConnectionAttempsts++;
				_oGatewayEndpoint.debug( '[ Endpoint ( ID: %s ) ] Creating Emitter from options: "%j" ( Attempt number: "%s" )', _oGatewayEndpoint.getID(), this.__oOptions, this.__iConnectionAttempsts );
				var _oEmitter = new Emitter( this.__oOptions );
				//Generic events
				_oEmitter.on( 'listening', function() {
					_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] is ready...', _oGatewayEndpoint.getID() );
					_oGatewayEndpoint.__bReady = true;
					_oGatewayEndpoint.emit( 'ready' );
				});
				_oEmitter.on( 'connect', function() {
					_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] connection estabilished with "%s:%s"...', _oGatewayEndpoint.getID(), _oGatewayEndpoint.getHost(), _oGatewayEndpoint.getPort() );
					_oGatewayEndpoint.__bReady = true;
					_oGatewayEndpoint.emit( 'connect' );
					_oGatewayEndpoint.emit( 'ready' );
				});
				_oEmitter.on('data', function( oData, iSocketIndex ) {
					_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] Data received from socket with index "%s": "%s"...', _oGatewayEndpoint.getID(), iSocketIndex, oData.toString('hex') );
					_oGatewayEndpoint.emit('data', oData, iSocketIndex );
				});
				_oEmitter.on('drain', function( oData ) {
					_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] Data drained: "%s"...', _oGatewayEndpoint.getID(), oData.toString('hex') );
					_oGatewayEndpoint.emit('drain', oData );
				});
				_oEmitter.on('timeout', function() {
					_oGatewayEndpoint.debug( '[ Endpoint ( ID: %s ) ] Timeout', _oGatewayEndpoint.getID() );
					_oGatewayEndpoint.emit('timeout' );
				});
				_oEmitter.on('close', function( iSocketIndex ) {
					_oGatewayEndpoint.debug( '[ Endpoint ( ID: %s ) ] Socket "%j" closed', _oGatewayEndpoint.getID(), iSocketIndex );
				...
				_oEmitter.on('error', function( oError ) {
					if( oError.code === 'ECONNREFUSED' || oError.code === 'ETIMEDOUT' ){
						...
						if( _bCanReconnect ){
							_oGatewayEndpoint.info( '[ Endpoint ( ID: "%s" ) ] Re-connection attempt number:"%s" in "%s" seconds.', _oGatewayEndpoint.getID(), ( _oGatewayEndpoint.__iConnectionAttempsts + 1 ) , _oGatewayEndpoint.__oOptions.reconnectionDelay );
							...
							_oGatewayEndpoint.info( '[ Endpoint ( ID: "%s" ) ] failed all allowed attempts to reconect. %s', _oGatewayEndpoint.getID(), oError );
						}

					} else {
						_oGatewayEndpoint.error( '[ Endpoint ( ID: "%s" ) ] %s', _oGatewayEndpoint.getID(), oError );
					}
				...
				switch( this.getType() ){
					case 'listen':
						...
						_oEmitter.on('connection', function( oSocket ) {
							...
							_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] connection received from "%s:%s"...', _oGatewayEndpoint.getID(), _oSocket.remoteAddress, _oSocket.remotePort );
							...
							oSocket.on('data', function( oData ) {  // client writes message
								_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] Data received: "%s" from connected client "%s:%s"...', _oGatewayEndpoint.getID(), oData.toString('hex'), _oSocket.remoteAddress, _oSocket.remotePort );
							...
							oSocket.on('drain', function( oData ) {  // client writes message and leave empty place
								_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] Data drained: "%s" from connected client "%s:%s"...', _oGatewayEndpoint.getID(), oData.toString('hex'), _oSocket.remoteAddress, _oSocket.remotePort );
								_oEmitter.emit( 'drain', oData );
							});
							// Socket Event: end
							oSocket.on('end', function() { // client disconnects
								_oGatewayEndpoint.debug('[ Endpoint ( ID: %s ) ] Socket "%s" ( "%s:%s" ) has been disconnected...', _oGatewayEndpoint.getID(), _iSocketIndex, _oSocket.remoteAddres, _oSocket.remotePort );
				...
				_oGatewayEndpoint.error( '[ Endpoint ( ID: %s ) ] missing class emitter for connection type "%s"...', _oGatewayEndpoint.getID(), this.getConnectionType()  );
			}
		} else {
			// Recreating Emitter
	...
	setConnectedSocketID( iIndex, sID ){
		...
		if( !_sCurrentID ){
			this.debug( '[ Endpoint ( ID: "%s" ) ] connected socket with index: "%s" is now known as "%s"... ', this.getID(), iIndex, sID );
			this.__oConnectedSocketsID[ sID ] = iIndex;
		} else {
			this.error( '[ Endpoint ( ID: "%s" ) ] already known a connected socket with index: "%s" as "%s"; ignoring operation set ID "%s"... ', this.getID(), iIndex, _sCurrentID, sID );
		}
	}
	/**
	...
	*   Endpoint.getConnectedSocket( 1 );
	...
	write( oData, sEncoding, fCallback ){
		switch( this.getType() ){
			case 'connect':
				this.debug( '[ Endpoint ( ID: "%s" ) ] writing: "%s"... ', this.getID(), oData.toString('hex') );
			...
			case 'listen':
				...
				for( let _oConnectedSocket of _aConnectedSockets ){
					this.debug( '[ Endpoint ( ID: "%s" ) ] Writing on connected socket: "%s:%s"', this.getID(), _oConnectedSocket.remoteAddress, _oConnectedSocket.remotePort );
	...
	__canReconnect(){
		...
		if( _bResult ){
			this.debug( '[ Endpoint ( ID: "%s" ) ] Re-connection enabled.', this.getID() );
			...
			if( this.__oOptions.reconnectionAttempts > 0 ){
				...
				_bResult = true;
				this.debug( '[ Endpoint ( ID: "%s" ) ] can be re-connected without limits in the attempts.', this.getID() );
			}
		} else {
			this.debug( '[ Endpoint ( ID: "%s" ) ] Re-connection disabled.', this.getID() );
		}
		return _bResult;
	}
	/**
