...
var Technology=function( oTechnologyOptions ){
	...
	oTechnologyOptions = Tools.extend({
		...
		sType: 'unknown',
		bUseSwap: true,
		sDBPath: '.', // DB persistent path should be where the script technology is located
		sDBPathTmp: ( Tools.isOSWin() ? '.' : '/tmp' ), // TODO: DB volatile path should be located on TMP under UNIX or ? under Windows
		sLogPath: '.', // Log persistent path should be where the script technology is located
		sLogPathTmp: ( Tools.isOSWin() ? '.' : '/var/log' ), // TODO: Log volatile path should be located on TMP under UNIX or ? under Windows
		bUseDB: true,
		bUseLog: true,
	...
	var _sTechnologyFilename = oTechnologyOptions.sType + '.' + oTechnologyOptions.sID;
	oTechnologyOptions = Tools.extend({
		sDBName: _sTechnologyFilename + '.sqlite',
	...
	if( this.__oOptions.bUseLog ){
		winston.loggers.add('log-rotation', {
	    ...
	    file: {
				timestamp: true,
				filename: this.__oOptions.sLogPathTmp + '/' + _sTechnologyFilename + '.log',
...
Technology.prototype.__initDB = function( fCallback ){
	if( this.__oOptions.bUseDB ){
		this.__swapDB( this.__DBgetRelativePersistantPath(), this.__DBgetRelativePath(), Tools.proxy(
				this.__DBCheckUpdate, this, fCallback
			)
		);
	} else {
