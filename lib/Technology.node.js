...
Technology.prototype.run = function( oTechnologyOptions, oCurrentModule ){
	...
	oTechnologyOptions = Tools.extend({
		...
		sDBPath: 'DB/', // DB persistent path should be where the script technology is located ( if using relative path it's relative to sCwd )
		sDBPathTmp: ( Tools.isOSWin() ? 'DB/' : '/tmp/' ), // TODO: DB volatile path should be located on TMP under UNIX or ? under Windows ( if using relative path it's relative to sCwd )
		...
		sLogPathTmp: ( Tools.isOSWin() ? './' : '/var/log/' ), // TODO: Log volatile path should be located on TMP under UNIX or ? under Windows ( if using relative path it's relative to sCwd )
		bUseDB: true,
		bUseLog: true,
	...
	if( this.__oOptions.bUseLog ){
		...
		this.info( 'Logging technology on: "%s"...', winston.loggers.get('log-rotation').transports.file.filename );
	}
	...
	this.__registerEvents( Tools.proxy(
		this.__initDB, this, Tools.proxy(
...
Technology.prototype.__initDB = function( fCallback ){
	if( this.__oOptions.bUseDB ){
		...
		var _oSequelizeManager = new SequelizeManager({
		    database: this.__oOptions.sTechnologyTag,
		    username: this.__oOptions.sDBUsername,
		    password: this.__oOptions.sDBPassword,
		    host: null,
		  },{
				logging: function( sLog ){ _Technology.debug( sLog ); },
		    dialect: 'sqlite',
		    storage: this.__oOptions.sDBPath + this.__oOptions.sDBName
		  }
		);
		// Init Sequelize
		var _oSequelize = _oSequelizeManager.sequelize;
		/*
		var _oSequelize = new Sequelize( this.__oOptions.sTechnologyTag, this.__oOptions.sDBUsername , this.__oOptions.sDBPassword,{
			logging: function( sLog ){ _Technology.debug( sLog ); },
	    dialect: 'sqlite',
	    storage: this.__oOptions.sDBPath + this.__oOptions.sDBName
		});
		*/
		// Init DB's models
		var _sRelativeModelPath = this.__oOptions.sDBPath + 'models';
		var _oDBModels = {};
		Fs
		  ...
		  .forEach( function( sFile ){
		    var _oModel = _oSequelize.import( Path.join( process.cwd(), _sRelativeModelPath, sFile ) );
		...
		*/
		// Umzug
		var _oUmzug = new Umzug({
		  storage: 'sequelize',
		  storageOptions: {
		      sequelize: _oSequelize,
		  },
		  logging: function( sLog ){ _Technology.debug( 'Migrating DB: %j', sLog ); },
		  migrations: {
		    params: [ _oSequelize.getQueryInterface(), _oSequelize.constructor, function(){ throw new Error('Migration tried to use old style "done" callback. Please upgrade to "umzug" and return a promise instead.'); }],
		    path: this.__oOptions.sDBPath + 'migrations',
		    pattern: /\.js$/
		  }
		});
...
/**
 * Method called to check if the DB version needs to be updated to the latest one
 *
 * @method    __DBCheckUpdate
 * @private
 *
 * @param	{Function}	fCallback		The callback function to call when the initialize DB operation is completed
 *
 * @example
 *   Technology.__DBCheckUpdate( fCallback );
 */
/*
Technology.prototype.__DBCheckUpdate = function( fCallback ){
	var _Technology = this;
