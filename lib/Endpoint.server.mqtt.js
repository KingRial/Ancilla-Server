 ...
 //*  - `publishSubscriptions`, publish message to topic "$SYS/{broker-id}/new/(un)subscribes" when a client subscribes/unsubscribes.
    ...
    if( this.__oOptions.sSSLKey && this.__oOptions.sSSLCert ){
      ...
      _oMoscaOptions.interfaces[ 0 ].credentials = {
        ...
        caPaths: this.__oOptions.aSSLCA || [],
        requestCert: true, // Request certificate from client to be checked
        rejectUnauthorized: true // Error on self-signed certificates
      };
    }
    // WS/WSS protocol
    if( this.__oOptions.bUseWS ){
      _oMoscaOptions.interfaces.push( {
        type: ( ( this.__oOptions.sSSLKey && this.__oOptions.sSSLCert ) ? 'https' : 'http' ),
        credentials: ( ( this.__oOptions.sSSLKey && this.__oOptions.sSSLCert ) ? {
          keyPath: this.__oOptions.sSSLKey,
          certPath: this.__oOptions.sSSLCert,
          caPaths: this.__oOptions.aSSLCA || [],
// TODO: disabled until I do not understand how it works ( for some reason chrome doesn't send the correct certificate to the server )
          requestCert: false, // Request certificate from client to be checked
          rejectUnauthorized: false // Error on self-signed certificates
        }: null ),
