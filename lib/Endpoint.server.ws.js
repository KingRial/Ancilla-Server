...
class WSServer extends EndpointServer{
  ...
  init(){
    ...
    // Listening
    let _oServer = _oExpress.listen( this.__oOptions.port );
    // Create a Socket.IO instance, passing it our server
    let _oWebSocket = SocketIO.listen( _oServer );
    */
    //let _oExpress = express.createServer();
    //let _oWebSocket = SocketIO( _oExpress );
    let _oExpress = express();
    ...
    if( this.__oOptions.sWWW ){
      _Server.debug('Serving static content from: "%s" on port "%s"', this.__oOptions.sWWW, this.__oOptions.port );
      _oExpress.use( express.static( this.__oOptions.sWWW ) );
    }
    // Listening
    _oServer.listen( this.__oOptions.port );
    // Adding events
    _Server.debug('Serving websocket on port "%s" and path "%s"', this.__oOptions.port, this.__oOptions.sWsPath );
    _oWebSocket
