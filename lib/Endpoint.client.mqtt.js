...
class EndpointClientMQTT extends EndpointClient {
  ...
  init(){
    let _Endpoint = this;
    let _aCAs = [];
    if( this.__oOptions.aCA ){
      this.__oOptions.aCA.forEach( function( sFile, iIndex ){
        _aCAs[ iIndex ] = fs.readFileSync( sFile, 'utf8');
      });
    }
    let _oMQTTClientOptions = {
      ...
      // MQTTS options ( https://nodejs.org/api/tls.html#tls_tls_connect_options_callback )
      passphrase: this.__oOptions.sCertPassphrase,
      cert: ( this.__oOptions.sCert ? fs.readFileSync( this.__oOptions.sCert, 'utf8') : null ),
      key: ( this.__oOptions.sCertKey ? fs.readFileSync( this.__oOptions.sCertKey, 'utf8') : null ),
      ca: ( _aCAs.length!== 0 ? _aCAs : null ),
      //requestCert: false,
      rejectUnauthorized: false // Needed to ignore errors on connecting on MQTT's server with self-signed CA
    };
    let _oClient  = MQTT.connect( this.getURL(), _oMQTTClientOptions );
  ...
  subscribe( sTopic ){
    ...
    return new Bluebird( function( fResolveSubscribe, fRejectSubscribe ){
      _Endpoint.__oEndpoint.subscribe( sTopic, function( error, aGranted ){
        if( error || ( _.isArray( aGranted ) && aGranted[ 0 ].qos === 128 ) ){
          // Qos 128: http://www.hivemq.com/blog/mqtt-essentials-part-4-mqtt-publish-subscribe-unsubscribe
          error = ( error ? error : new Error( 'Qos 128' ) );
          _Endpoint.error( 'Unable to subscribed on Topic "%s": %s', sTopic, error );
          fRejectSubscribe( error );
        } else {
          _Endpoint.info( 'Subscribed on Topic "%s"', sTopic );
          fResolveSubscribe();
        }
      } );
    });
  ...
  __writeOnEndpoint( oBuffer, oOptions ){
    ...
    if( oOptions.sTopic ){
      _aPublishPromises.push( new Bluebird( function( fResolve, fReject ){
        // You can plublish only strings!!!
        _Client.__oEndpoint.publish( oOptions.sTopic, oBuffer.toString(), function( error ){
          if( error ){
            _Client.error( 'Failed to publish "%s" on topic "%s": %s', oBuffer.toString(), oOptions.sTopic, error );
            fReject( error );
          } else {
            _Client.info( 'Published "%s" on Topic "%s"', oBuffer.toString(), oOptions.sTopic );
            fResolve();
          }
        } );
      }) );
      ...
      _aTopics.forEach(function( sTopic ){
        _aPublishPromises.push( new Bluebird( function( fResolve ){
          ...
          _Client.__oEndpoint.publish( sTopic, oBuffer.toString(), function(){
            _Client.info( 'Published "%s" on Topic "%s"', oBuffer.toString(), sTopic );
            fResolve();
          } );
        }) );
      });
