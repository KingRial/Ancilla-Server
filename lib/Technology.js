...
class Technology extends EventEmitter {
	...
	*/
	onAncilla( oEvent, oEndpoint, sSocketID, aPromisesToHandle ){
		...
		if( ( !oEvent.isSource( this.getID() ) ) && oEvent.isTarget( this.getID() ) && oEvent.isRequest() ){
			this.debug( 'Received Ancilla Event "%s" reached destination, handling request...',  oEvent.getID() );
			let _Technology = this;
			// Main promise
			...
			this.debug( 'Received Ancilla Event "%s" is not for current technology ( event source: "%s" event target: "%s" ), trying to dispatch it somewhere else...', oEvent.getFrom(), oEvent.getTo(), oEvent.getID() );
