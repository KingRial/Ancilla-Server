...
class Technology extends EventEmitter {
	constructor( oTechnologyOptions ){
		...
		this.configDefault( _oDefaultOptions );
		// Config options
		this.config( oTechnologyOptions );
	}

	configDefault( oDefaultOptions ){
		// Merging curren default options with default options from argument
		let oMergedDefaultOptions = _.extend( this.__oOptionsDefault, oDefaultOptions );
		// Calculating new options ( if needed ) and storing result
		this.__oOptionsDefault = this.__fillConfig( oDefaultOptions, oMergedDefaultOptions );
	}

	config( oOptions ){
		if( _.isObject( oOptions ) ){
			let oMergedOptions = ( _.isEmpty( this.__oOptions ) ? _.extend( this.__oOptionsDefault, oOptions ) : _.extend( this.__oOptions, oOptions ) );
			this.__oOptions = this.__fillConfig( oOptions, oMergedOptions );
		}
	}

	__fillConfig( oSourceOptions, oMergedOptions ){
		if( oSourceOptions.sType || oSourceOptions.sID ){
			let _sTechnologyTag = oMergedOptions.sType + '.' + oMergedOptions.sID;
			oMergedOptions = _.extend( oMergedOptions, {
				sTechnologyTag: _sTechnologyTag,
				sDBName:  _sTechnologyTag + '.sqlite',
				sLogName: _sTechnologyTag + '.log'
			} );
		}
		if( oSourceOptions.sAssetsPath ){
			oMergedOptions.sAssetsPath = ( Path.isAbsolute( oSourceOptions.sAssetsPath ) ? oSourceOptions.sAssetsPath : Path.join( oSourceOptions.sCwd, oSourceOptions.sAssetsPath ) );
		}
	...
	run( oTechnologyOptions, oCurrentModule ){
		...
		this.config( _.extend( oTechnologyOptions, this.__argsToOptions() ) );
						...
						if( _Technology.__oEndpoints.hasOwnProperty( _sEndpoint ) ){
							...
							if( _oEndpointData.module ){
								...
								try {
									...
									_Technology.error( 'Unable to load Endpoint\'s module library for "%s (%s)"\nPlease use a standard Endpoint\'s module library or use the Endpoint\'s paramater "module" to use a custom module', _oEndpointData.id, _oEndpointData.type );
									fReject( e );
								}
							}
	...
	ready(){
		...
		return this.__initAncillaEvents() // Init Ancilla Events
			...
			.then ( function(){ // Init Endpoints
				return _Technology.__initEndpoints( _Technology.__oOptions.oEndpoints );
			})
			.catch( function( oError ){
