...
class Technology extends EventEmitter {
			...
			case 'ancilla':
				...
				for( let _oAncillaEvent of _aAncillaEvents ){
					...
					if( _oAncillaEvent.isTarget( this.getID() ) ){
						...
						this.debug( 'Ignoring ancilla event: the event\'s target "%s" is not the current technology "%s"', _oAncillaEvent.getTo(), this.getID() );
					}
				}
			break;
			default:
	...
	trigger( oEventOptions, oOptions ){
		oEventOptions = _.extend({
			sFromID: this.getID(),
			sToID: this.getCoreEndpoint().getID()
		}, oEventOptions );
		oOptions = _.extend({
		...
		this.write( ( _oAncillaEvent.isAnswer() ? _oAncillaEvent.getFrom() : _oAncillaEvent.getTo() ), _oAncillaEvent, oOptions );
		// Returning Promise
	...
	*/
	__clearDeferredAncillaRequest( oAncillaEvent, bHandlePromiseAnswer ){
		...
		if( oAncillaEvent.isAnswer() ){
			...
			this.warn( 'Unable to clear deferred ancilla request "%s"; the request doesn\'t need an aswer', _iAncillaEventID );
		}
	}
				...
				_aPromisesToHandle.push(
					new Bluebird( function( fResolve, fReject ){
						...
						if( typeof _fAncillaEvent === 'function' ){
							let _oResult = _fAncillaEvent( _Technology, oEvent );
							// Adding automatic answer if needed
							if( oEvent.needsAnswer() ){
								if( typeof _oResult === 'object' && _oResult.then ){
									_oResult
										.then( function(){
					            _Technology.trigger( oEvent.toAnswer( Constant._NO_ERROR ) );
					          })
					        ...
					        ;
									...
									_Technology.warn('Ancilla Event "%s" doesn\'t return a promise; unable to handle requested answer automatically; please correct the event or handle answer programmatically ', oEvent.getID() );
								}
								return _oResult;
							}
						} else {
	...
	*/
	__onAncillaDispatch( oEvent ){
		this.warn( 'Still need to create the dispatch method...', oEvent );
