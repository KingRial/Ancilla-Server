...
class Technology extends EventEmitter {
	...
	__fillConfig( oNewOptions ){
		...
		if( oNewOptions.sAssetsPath ){
			_oOptions.sAssetsPath = ( Path.isAbsolute( oNewOptions.sAssetsPath ) ? oNewOptions.sAssetsPath : Path.join( oNewOptions.sCwd, oNewOptions.sAssetsPath ) );
		}
		return _oOptions;
	}
		...
		switch( sEvent ){
			case 'ancilla':
				...
				for( let _oAncillaEvent of _aAncillaEvents ){
					...
					if( _oAncillaEvent.getTo() === this.getID() ){
						...
						this.debug( 'Ignoring ancilla event: "%s"', _oAncillaEvent );
					}
				}
			break;
			default:
	...
	*/
	onAncilla( oEvent, oEndpoint, sSocketID, aPromisesToHandle ){
		this.debug( 'Received from entpoind "%s" and socket "%s" Ancilla Event: ',  oEndpoint.getID(), sSocketID, oEvent );
