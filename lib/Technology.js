...
class Technology extends EventEmitter{
				...
				for( let _sEndpoint in oEndpoints ){
					if( oEndpoints.hasOwnProperty( _sEndpoint ) ){
						if( _Technology.__oEndpoints.hasOwnProperty( _sEndpoint ) ){
							...
							let _Endpoint = null;
							let _sEndpointLib = 'Endpoint.' + _oEndpointData.type + '.js';
							let _sEndpointAssetsPath = Path.join( _Technology.__oOptions.sAssetsPath, 'lib/', _sEndpointLib );
							let _sEndpointLibCorePath = './' + _sEndpointLib;
							try {
								_Endpoint = require( _sEndpointAssetsPath );
							} catch( e ) {
								try{
									_Endpoint = require( _sEndpointLibCorePath );
								} catch( e ) {
									_Technology.error( 'Unable to load Endpoint library "%s"\nLooked inside "%s" and default library paths "%s".', _sEndpointLib, _sEndpointAssetsPath, _sEndpointLibCorePath );
									fReject( e );
								}
							}
	...
	__initEndpoint( oEndpoint ){
		...
		oEndpoint.on('datagram', function( oDatagram, oParsedBuffer, sSocketID ) {
			_Technology.emit( 'datagram', oDatagram, oParsedBuffer, oEndpoint, sSocketID );
		});
