  ...
  __initEventReconnect( oError ){
    ...
    if( oError.code === 'ECONNREFUSED' || oError.code === 'ETIMEDOUT' ){
      ...
      if( _Client.__canReconnect() ){
        ...
        _Client.info( 'Re-connection attempt number:"%s" in "%s" seconds.', _Client.__iConnectionAttempts, _Client.__oOptions.reconnectionDelay );
	...
	__canReconnect(){
		...
		if( _bResult ){
			...
			if( this.__oOptions.reconnectionAttempts > 0 ){
				_bResult = ( this.__oOptions.reconnectionAttempts < this.__iConnectionAttempts ? true : false );
			} else {
				_bResult = true;
