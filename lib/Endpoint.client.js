...
class EndpointClient extends Endpoint{
  ...
  init( oClient ){
    ...
    if( this.__oEndpoint ){
      this.__oEndpoint.on('close', function(){
        _Client.debug( 'Closing client', _Client.__oEndpoint.address() );
        _Client.emit( 'close' );
      });
      this.__oEndpoint.on('data', function( oBuffer ){
        _Client.debug( 'Client received data "%s"', oBuffer.toString('hex') );
        _Client.emit( 'data', oBuffer );
      });
      this.__oEndpoint.on('drain', function(){
        _Client.debug( 'Client drain event' );
        _Client.emit( 'drain' );
      });
      this.__oEndpoint.on('end', function(){
        _Client.debug( 'Client end event' );
        _Client.emit( 'end' );
      });
      this.__oEndpoint.on('lookup', function(){
        _Client.debug( 'Client lookup event' );
        _Client.emit( 'lookup' );
      });
      this.__oEndpoint.on('timeout', function(){
        _Client.debug( 'Client timeout event' );
  ...
  connect(){
    ...
    this.__oPromiseReady = new Bluebird(function( fResolve, fReject ){
      ...
      _Client.debug( 'Client connecting with following options:', _oConnectOptions );
  ...
  /**
	 * Method used to write a buffer on the current socket
	 *
	 * @method    write
	 * @public
	 *
   ...
   *
	...
	write( oData, sEncoding ){
		this.silly( 'writing: "%s"... ', oData.toString('hex') );
    let _Client = this;
    return new Bluebird(function( fResolve ){
  		_Client.__oEndpoint.write( oData, sEncoding, function(){
        _Client.silly( 'written: "%s"... ', oData.toString('hex') );
        fResolve();
      } );
    });
	}
