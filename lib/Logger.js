...
class Logger {
  constructor( oLoggerOptions, oTargetToExtend ){
    ...
    var _oLogOptions = {
			console: {
				level: oLoggerOptions.sLevel,
				silent: oLoggerOptions.bSilent,
        colorize: true,
        prettyPrint: true,
        timestamp: true
			}
		};
    if( oLoggerOptions.sLogPath ){
			_oLogOptions.file = {
        level: oLoggerOptions.sLevel,
				filename: oLoggerOptions.sLogPath,
				maxsize: oLoggerOptions.iLogMaxSize, // 500 kB
				maxFiles: oLoggerOptions.iLogMaxFiles,
        prettyPrint: true,
        timestamp: true,
				json: false,
				tailable: true,
    ...
    if( _oLogOptions.file ){
			this.info( 'Log ID "%s" logging on console and file: "%s"...', this.getID(), _oLogOptions.file.filename );
		} else {
